...
from .models import FMGenModel, FMGenDecoder
...
import warnings
# Optional import for igl (only needed for curvature computation)
try:
    import igl
    HAS_IGL = True
except ImportError:
...
###### compute princple############# VVVVVVVV
def compute_principal_curvature_igl(mesh: Trimesh):
    """
    Compute the principal curvature magnitudes and directions using LibIGL.
    
    Parameters:
        mesh (Trimesh): Input triangular mesh.
...
def save_principal_curvature(mesh_paths: List[str], save_path: str):
    """
...
def save_curvatures_for_dataset(config):
    """
    Compute and save principal curvatures for all meshes in the dataset.

    Parameters:
        config (dict): Configuration containing paths and parameters.
    """
    dataset_dir = config["dataset_dir"]
    curvature_save_path = join(dataset_dir, "curvature_data.pkl")
    mesh_paths = [join(dataset_dir, f) for f in os.listdir(dataset_dir) if f.endswith(".obj")]

    save_principal_curvature(mesh_paths, curvature_save_path)
    print(f"Principal curvatures saved at: {curvature_save_path}")
...
def generate_VFDU(mesh: Trimesh, factors: List[float]):
    ...
    for factor in factors:
        # Simplify mesh using quad_mesh_simplify
...
def load_generator(config):
    ...
    # Create the full model (encoder + decoder)
    model = FMGenModel(config, A, D, U)
    
    # Load encoder state dict
    encoder_state_dict = torch.load(os.path.join(config["checkpoint_dir"], "checkpoint_encoder.pt"), map_location='cuda')
    model.encoder.load_state_dict(encoder_state_dict)
    
    # Load decoder state dict
    decoder_state_dict = torch.load(os.path.join(config["checkpoint_dir"], "checkpoint_decoder.pt"), map_location='cuda')
